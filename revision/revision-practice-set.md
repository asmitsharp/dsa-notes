...
Example reflection for `Running Sum of 1d Array`:

- Trigger: We need the cumulative total at every index, so prefix sum is the right pattern.
- Invariant: `prefix[i]` always stores the sum of `nums[0..i]`, so `prefix[i] = prefix[i - 1] + nums[i]`.
- Time complexity: `O(n)`.
- One mistake you made or almost made: I first forgot the prefix sum formula. After that, I almost introduced an out-of-range bug by not handling the first element separately. `result` should be created with size `len(nums)`, and `result[0] = nums[0]` should be set before looping from index `1` instead of starting with `result := []int{}`.
